...
<body class="bg-gray-50">
    ...
    <div class="flex gap-5">
        ...
        $sql = "SELECT pertemuan_ke FROM bimbingan ORDER BY pertemuan_ke DESC LIMIT 1";
        $stmt = $db->prepare($sql);
        $stmt->execute();
        ...
        $stmt = $db->prepare($sql);
        $stmt->execute();
        $listDosenPa = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ?>
        ...
        <div class="m-20 ">
            ...
            <div class="flex gap-2 my-5">
                <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                    ...
                    <!-- loop list dosen -->
                    <?php foreach ($listDosenPa as $key => $dosenpa) : ?>
                        <?php
                        $sql = "SELECT bimbingan.*, mahasiswa.nama AS nama, mahasiswa.nim AS nim FROM bimbingan
                            ...
                            WHERE bimbingan.id_dosen=$dosenpa[id] GROUP BY bimbingan.id_mahasiswa";
                        $stmt = $db->prepare($sql);
                        $stmt->execute();
                        $bimbingan = $stmt->fetchAll(PDO::FETCH_ASSOC);
                        ?>

                        <div class="p-5 m-2 border shadow shadow-lg bg-success border-black/25 rounded-xl w-60 border-1">
                            <a href="#">
                                <h5 class="mb-2 text-xl font-bold tracking-tight text-gray-900 dark:text-white"><?= $dosenpa['nama'] ?></h5>
                            </a>
                            <p class="text-gray-700 dark:text-gray-400"><?= $dosenpa['nip'] ?></p>
                        </div>

                        <table class="w-full my-8 text-sm text-left text-gray-500 dark:text-gray-400">
